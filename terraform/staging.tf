resource "aws_opsworks_stack" "apr-staging" {
    name = "apr-staging"
    region = "us-east-1"
    ...
    default_subnet_id = "${terraform_remote_state.infrastructure.output.vpc_staging_private_subnet_1b_id}"
    use_custom_cookbooks = "1"
    vpc_id = "${terraform_remote_state.infrastructure.output.vpc_staging_id}"
    custom_json = "${file("${path.module}/staging.json")}"
...
resource "aws_opsworks_custom_layer" "apr-backend" {
    ...
    stack_id = "${aws_opsworks_stack.apr-staging.id}"
    custom_security_group_ids = ["${terraform_remote_state.infrastructure.output.vpc_staging_default_sg_id}"]
    elastic_load_balancer = "${aws_elb.apr-staging-http.name}"
...
resource "aws_opsworks_application" "apr-staging" {
    name = "apr"
    stack_id = "${aws_opsworks_stack.apr-staging.id}"
    type = "other"
}

resource "aws_elb" "apr-staging-http" {
    name                        = "apr-staging-http"
    subnets                     = [
